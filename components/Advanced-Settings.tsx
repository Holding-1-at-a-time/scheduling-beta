...
export default function AdvancedSettings({ tenantId }: AdvancedSettingsProps) {
  ...
  const advancedSettings = useQuery(api.advancedSettings.get, { tenantId });
  const updateAdvancedSettings = useMutation(api.advancedSettings.update);
  const exportData = useMutation(api.advancedSettings.exportData);
  ...
  const onSubmit = async (data: AdvancedSettingsForm) => {
    ...
    try {
      await updateAdvancedSettings({ tenantId, settings: data });
      toast({
        title: "Success",
